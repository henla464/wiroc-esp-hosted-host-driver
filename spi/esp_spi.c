...
static int process_rx_buf(struct sk_buff *skb)
{
	...
	esp_info("--- New RX packet---");
	esp_info("--- ZERO packet received ---");
		return -EINVAL;
	}
	len = le16_to_cpu(header->len);
	len += sizeof(struct esp_payload_header);
	esp_info("--- New RX packet--- %d",len);
	esp_hex_dump("RX packet: ", skb->data, min((u32)len, 32u));
	...
	if (!len) {
		esp_err("len is 0, drop\n");
		return -EINVAL;
	}


	if (len > SPI_BUF_SIZE) {
		esp_err("len %d > SPI_BUF_SIZE %d, drop\n", len, SPI_BUF_SIZE);
		return -EINVAL;
	}
...
static void esp_spi_work(struct work_struct *work)
{
	...
	if (trans_ready) {
		...
		if (rx_pending || tx_skb) {
			...
			if (tx_skb) {
				esp_info("--- New TX packet %d---", tx_skb->len);
				esp_hex_dump("TX packet: ", tx_skb->data, min((u32)tx_skb->len, 32u));
				trans.tx_buf = tx_skb->data;
			} else {
				esp_info("--- ZERO packet sent ---");
